...
Matrix4x4 Add(const Matrix4x4& m1, const Matrix4x4& m2) {
	...
	for (int i = 0; i < 4; i++)
	{
		result.m[i][0] = m1.m[i][0] + m2.m[i][0];
	}
	for (int i = 0; i < 4; i++)
	{
		result.m[i][1] = m1.m[1][0] + m2.m[i][0];
	}
	for (int i = 0; i < 4; i++)
	{
		result.m[i][2] = m1.m[i][2] + m2.m[i][2];
	}
	for (int i = 0; i < 4; i++)
	{
		result.m[i][3] = m1.m[i][3] + m2.m[i][3];
	}
	return result;
};

Matrix4x4 Subtract(const Matrix4x4& m1, Matrix4x4& m2) {
	Matrix4x4 result;
	for (int i = 0; i < 4; i++)
	{
		result.m[i][0] = m1.m[i][0] - m2.m[i][0];
...
Matrix4x4 Multiply(const Matrix4x4& m1, Matrix4x4& m2) {
	...
	for (int i = 0; i < 4; i++)
	{
		result.m[i][1] = m1.m[i][1] * m2.m[0][1] + m1.m[i][1] * m2.m[1][1] + m1.m[i][2] * m2.m[2][1] + m1.m[i][3] * m2.m[3][1];
	}
	for (int i = 0; i < 4; i++)
	{
		result.m[i][2] = m1.m[i][2] * m2.m[0][0] + m1.m[i][1] * m2.m[1][2] + m1.m[i][2] * m2.m[2][2] + m1.m[i][3] * m2.m[3][3];
	}
	for (int i = 0; i < 4; i++)
	{
		result.m[i][3] = m1.m[i][3] * m2.m[0][0] + m1.m[i][3] * m2.m[1][0] + m1.m[i][3] * m2.m[2][0] + m1.m[i][3] * m2.m[3][0];
	}
	return result;
};
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		MatrixScreenPrintf(0, kRowHeight * 5 * 2, resultMultiply, "Multipty");
		...
		MatrixScreenPrintf(kColumnWidth * 5, kRowHeight * 5 * 2, identity, "isentity");
